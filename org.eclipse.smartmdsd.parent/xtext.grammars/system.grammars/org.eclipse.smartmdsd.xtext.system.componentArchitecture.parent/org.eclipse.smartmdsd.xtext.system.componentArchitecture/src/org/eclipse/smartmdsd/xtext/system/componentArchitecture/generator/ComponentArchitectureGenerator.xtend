...
//
//================================================================
package org.eclipse.smartmdsd.xtext.system.componentArchitecture.generator
 ...
 */
class ComponentArchitectureGenerator extends AbstractGenerator {

	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		// get all generators plugged-in as extensions
		val IConfigurationElement[] config = Platform.getExtensionRegistry().getConfigurationElementsFor("org.eclipse.smartmdsd.xtext.system.componentArchitecture.ComponentArchitectureGeneratorEP")
		try {
			// for each extension
			for(ext: config) {
				...
				if(obj instanceof AbstractGenerator) {
					var ISafeRunnable runnable = new ISafeRunnable() {
						override void handleException(Throwable e) {
							println("ComponentArchitectureGenerator Exception")
							e.printStackTrace
						}
